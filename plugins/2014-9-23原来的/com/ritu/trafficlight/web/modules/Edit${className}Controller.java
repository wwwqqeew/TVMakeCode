...
public class Edit${className}Controller {
	...
	public String findOneById(@RequestParam(value = "id", required = false) <#list table.columns as column><#if (column.pk)>${java_p(column)}</#if></#list> id) {
	...
	public String show(@RequestParam("id")  <#list table.columns as column><#if (column.pk)>${java_p(column)}</#if></#list> id,ModelMap model) {
