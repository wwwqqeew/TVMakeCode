...
public class ${className}ManagementController {
	...
	public String delete(@RequestParam(value = "id", required = false) <#list table.columns as column><#if (column.pk)>${java_p(column)}</#if></#list> id) {
	...
	public String deleteByIds(
		...
		for (String x : theIds) {
			${classNameLower}Service.delete(<#list table.columns as column><#if (column.pk)>${java_p(column)}</#if></#list>.valueOf(x.trim()));
		}
		return JsonBuilder.toJson(true);
	}
