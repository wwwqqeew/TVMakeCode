...
setInputData = function(data) {
	data = (data || {});
	$("#edit-"+name+"-form").find(".<#list table.columns as column><#if (column.pk)>${column.columnNameLower}</#if></#list>").val(data.<#list table.columns as column><#if (column.pk)>${column.columnNameLower}</#if></#list> || "");
	<#list table.columns as column>
	<#if !column.pk>
	<@js column=column function="sm">
...
getInputData = function(data) {
	data = (data || {});
	data.<#list table.columns as column><#if (column.pk)>${column.columnNameLower}</#if></#list> = $("#edit-"+name+"-form").find(".<#list table.columns as column><#if (column.pk)>${column.columnNameLower}</#if></#list>").val();
	<#list table.columns as column>
	<#if !column.pk>
	<#assign used = 0>
	<@htm column=column function="radio_df">
